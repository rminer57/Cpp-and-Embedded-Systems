...
#pragma implementation "DeviceDS2505.h"
#endif

#include <serial_comp.h>
#include <DeviceDS2505.h>
#include <IOVirtual.h>
...
const unsigned char DeviceDS2505::CMD_WRITE_MEMORY( 0x0F );
// }}}RME

// {{{RME classAttribute 'CMD_WRITE_MEMORY_SIZE'
const unsigned int DeviceDS2505::CMD_WRITE_MEMORY_SIZE( 3 );
// }}}RME

// {{{RME classAttribute 'CMD_READ_STATUS'
const unsigned char DeviceDS2505::CMD_READ_STATUS( 0xAA );
// }}}RME

// {{{RME classAttribute 'CMD_READ_STATUS_SIZE'
unsigned int DeviceDS2505::CMD_READ_STATUS_SIZE( 3 );
// }}}RME

// {{{RME classAttribute 'CMD_READ_EPROM'
const unsigned char DeviceDS2505::CMD_READ_EPROM( 0xF0 );
// }}}RME

// {{{RME classAttribute 'CMD_READ_EPROM_SIZE'
const unsigned int DeviceDS2505::CMD_READ_EPROM_SIZE( 3 );
// }}}RME

// {{{RME classAttribute 'DS2505_MEMORY_SIZE'
const unsigned int DeviceDS2505::DS2505_MEMORY_SIZE( 2048 );
// }}}RME

// {{{RME classAttribute 'DS2505_SCRATCHPAD_SIZE'
const unsigned int DeviceDS2505::DS2505_SCRATCHPAD_SIZE( 1 );
// }}}RME

// {{{RME classAttribute 'CMD_WRITE_STATUS'
const unsigned char DeviceDS2505::CMD_WRITE_STATUS( 0x55 );
// }}}RME

// {{{RME classAttribute 'CMD_WRITE_STATUS_SIZE'
const unsigned int DeviceDS2505::CMD_WRITE_STATUS_SIZE( 4 );
// }}}RME

// {{{RME classAttribute 'DS2505_PAGE_SIZE'
const unsigned int DeviceDS2505::DS2505_PAGE_SIZE( 32 );
// }}}RME

// {{{RME classAttribute 'DS2505_STATUS_BASE_ADDRESS'
const uint16_t DeviceDS2505::DS2505_STATUS_BASE_ADDRESS( 0x0000 );
// }}}RME

// {{{RME classAttribute 'DS2505_STATUS_REDIRECT_OFFSET'
const uint16_t DeviceDS2505::DS2505_STATUS_REDIRECT_OFFSET( 0x0100 );
// }}}RME

// {{{RME classAttribute 'DS2505_PAGE_NUMBER_SHIFT'
const unsigned int DeviceDS2505::DS2505_PAGE_NUMBER_SHIFT( 5 );
// }}}RME

// {{{RME tool 'OT::Cpp' property 'GenerateDefaultConstructor'
DeviceDS2505::DeviceDS2505( void )
{
}
// }}}RME
...
unsigned char * DeviceDS2505::readMemory( unsigned int address, unsigned int length )
{
	// {{{USR
	...
	if ( (length > 0) && ((address + length) <= DS2505_MEMORY_SIZE) )
	{
		...
		while (!finished)
		{
			...
			pageOffset = (offset & (DS2505_PAGE_SIZE - 1));
			pageNumber = (offset & (~(DS2505_PAGE_SIZE - 1)));

			pageNumber >>= DS2505_PAGE_NUMBER_SHIFT;
			
			availablePageSize = DS2505_PAGE_SIZE - pageOffset;
			...
			statusRegisterAddress = DS2505_STATUS_BASE_ADDRESS +
									DS2505_STATUS_REDIRECT_OFFSET +
									(uint16_t)pageNumber;
			...
			else
			{
				...
				redirectionByte <<= DS2505_PAGE_NUMBER_SHIFT;
...
bool DeviceDS2505::writeMemory( unsigned int address, unsigned char * block, unsigned int length )
{
	// {{{USR
	bool retVal = false;
	bool finished = false;
	...
	if ( (length > 0) && ((address + length) <= DS2505_MEMORY_SIZE) )
	{
		...
		while(!finished)
		{
			scratchpadOffset = (offset & (DS2505_SCRATCHPAD_SIZE - 1));
			availableScratchpadSize = DS2505_SCRATCHPAD_SIZE - scratchpadOffset;
...
DeviceDS2505::DeviceDS2505( OneWireConnection * connection, IOVirtual * progPulse )
