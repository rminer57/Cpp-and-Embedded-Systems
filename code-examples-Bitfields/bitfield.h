// {{{RME classifier 'Logical View::rf_audio_subsystem::standard_includes::bitfield'
...
#define defineBitFieldCommon(name,type,off,start,len) \
	...
	const static unsigned int name ## _fieldMask = ((1 << (len))-1) << name ## _start; \
	const static unsigned int name ## _off = (((start)/PROCESSOR_ACCESS_SIZE) + (off)) / sizeof(type)
...
#define setBitField(base,name,value) \
	...
	if (name ## _reverse == 0) { \
		*((base) + name ## _off) = (*((base) + name ## _off) \
			& ~(name ## _fieldMask)) \
			| ((value) << name ## _start) & name ## _fieldMask; \
	} \
	if (name ## _reverse == 1) { \
		unsigned int temp = reverseEndian(*((base) + name ## _off),sizeof(*(base))); \
...
#define combineAddr(base,off) ((base) + ((off) / sizeof(*(base))))
